...
- Remove @appypeeps/clerk-react as a dev depedency. Move @clerk/shared to dependencies (previously devDepedencies). ([#4114](https://github.com/clerk/javascript/pull/4114)) by [@BRKalow](https://github.com/BRKalow)
...
- Change the `peerDependencies` range of `@appypeeps/clerk-react` and `@clerk/shared` to the stable Core 2 range
- Change the `peerDependency` range of `next` to mimic what `@clerk/nextjs` is doing
- Remove the `peerDependenciesMeta` entry for `@appypeeps/clerk-react` and `@clerk/shared` since that hacky workaround shouldn't be necessary anymore as Core 2 is stable now
